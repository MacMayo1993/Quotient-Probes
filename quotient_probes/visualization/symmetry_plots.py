...
from ..core.decomposition import compute_coherence, decompose
...
def plot_coherence_histogram(
    ...
    alphas = np.array([compute_coherence(x, sigma) for x in data_samples])

    fig, ax = plt.subplots(figsize=figsize)

    # Histogram
    counts, bins, patches = ax.hist(
        alphas, bins=n_bins, density=True, alpha=0.7, color="skyblue", edgecolor="black"
    )
    ...
    if show_critical and data_samples.shape[1] > 0:
        ...
        num_exploit = int(np.sum(alphas > alpha_crit))
        num_ignore = int(np.sum(alphas <= alpha_crit))
...
def plot_symmetry_test(
    ...
    ax1.plot(
        indices, -sigma_x, "g:^", label="-σ(x)", linewidth=2, markersize=6, alpha=0.7
    )
    ...
    ax2.plot(
        x_range, -x_range, "r--", linewidth=2, alpha=0.7, label="y = -x (antisymmetric)"
    )
