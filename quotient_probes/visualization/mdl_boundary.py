...
def plot_decision_boundary(
    ...
    ax.axhline(0, color='black', linestyle='--', linewidth=1.5, alpha=0.7, label='Decision boundary')

    # Shade regions
    ax.axvspan(0, 0.5, alpha=0.1, color='red', label='Ignore (α < 0.5)')
    ax.axvspan(0.5, 1.0, alpha=0.1, color='green', label='Potentially exploit (α > 0.5)')
...
def plot_interactive_boundary(
    ...
    slider_K = Slider(
        ax_K, 'Orientation Cost K_lift',
        K_lift_range[0], K_lift_range[1],
        valinit=K_lift_init,
        valstep=0.1
    )
...
def plot_dimension_sweep(
    ...
    for n_mark in [64, 256, 1024]:
        if n_mark <= n_max:
            ...
            ax.text(n_mark, 0.495, f'n={n_mark}', rotation=90,
