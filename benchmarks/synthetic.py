...
class SyntheticDataGenerator:
    ...
    def generate_with_coherence(
        ...
        if energy_plus > 0:
            x_plus = x_plus_raw * np.sqrt(energy_plus / (np.linalg.norm(x_plus_raw)**2 + 1e-10))
        else:
            x_plus = np.zeros(n)

        if energy_minus > 0:
            x_minus = x_minus_raw * np.sqrt(energy_minus / (np.linalg.norm(x_minus_raw)**2 + 1e-10))
        else:
            x_minus = np.zeros(n)
...
def benchmark_coherence_sweep(
    ...
    print(f"{'Target α':<12} {'Actual α':<12} {'Decision':<10} {'Expected':<10} {'Correct':<8} {'Bit Savings':<12}")
    print("-" * 80)
    ...
    for i, target_alpha in enumerate(alphas_target):
        ...
        print(f"{target_alpha:<12.3f} {actual_alpha:<12.4f} {exploit_str:<10} {expected_str:<10} {marker:<8} {bit_savings:+12.2f}")

    accuracy = correct_decisions / num_points
    runtime = time.time() - start_time

    print(f"\n{'='*70}")
    print(f"RESULTS: {correct_decisions}/{num_points} correct ({accuracy*100:.1f}% accuracy)")
    ...
    if save_plot:
        ...
        ax.axvline(alpha_crit, color='r', linestyle='--', linewidth=1.5, label=f'α_crit={alpha_crit:.4f}')

        # Mark test points
        for r in results:
            color = 'green' if r['should_exploit'] else 'red'
            ax.plot(r['actual_alpha'], description_length_difference(r['actual_alpha'], n, K_lift),
        ...
        # Plot 3: Bit savings
        ax = axes[1, 0]
        bit_savings = [r['bit_savings'] for r in results]
        colors = ['green' if s > 0 else 'red' for s in bit_savings]
        ax.bar(range(len(bit_savings)), bit_savings, color=colors, alpha=0.6)
        ...
        ax.bar(range(len(correct)), correct, color=['green' if c else 'red' for c in correct], alpha=0.6)
        ax.set_xlabel('Test case index', fontsize=11)
        ...
        plt.tight_layout()
        plt.savefig('benchmarks/results/coherence_sweep.png', dpi=150, bbox_inches='tight')
    ...
    return BenchmarkResult(
        ...
        metrics={
            'accuracy': accuracy,
            'avg_bit_savings': np.mean([r['bit_savings'] for r in results if r['should_exploit']]),
...
def benchmark_dimension_sweep(
    ...
    print(f"{'n':<8} {'α_crit':<10} {'Decision':<10} {'Bit Savings':<15} {'Runtime (ms)':<15}")
    print("-" * 70)

    for n in n_values:
        # Generate data
        x = SyntheticDataGenerator.generate_with_coherence(n, target_alpha, involution='reverse', seed=42)
        ...
        print(f"{n:<8} {alpha_crit:<10.4f} {decision:<10} {bit_savings:+15.2f} {runtime_ms:<15.3f}")
    ...
    if save_plot:
        ...
        ax.axhline(0.5, color='r', linestyle='--', linewidth=1.5, label='Asymptotic limit (0.5)')
        ...
        ax = axes[0, 1]
        bit_savings = [r['bit_savings'] for r in results]
        ax.semilogx(ns, bit_savings, 'o-', linewidth=2, markersize=8, color='green')
        ax.axhline(0, color='k', linestyle='-', linewidth=1)
        ax.set_xlabel('Dimension n (log scale)', fontsize=11)
        ax.set_ylabel('Bit savings', fontsize=11)
        ...
        plt.tight_layout()
        plt.savefig('benchmarks/results/dimension_sweep.png', dpi=150, bbox_inches='tight')
    ...
    return BenchmarkResult(
        ...
        metrics={
            'max_n': max(n_values),
            'max_bit_savings': max([r['bit_savings'] for r in results]),
            'avg_runtime_ms': np.mean([r['runtime_ms'] for r in results]),
        },
        success=True,
...
def benchmark_compression(
    ...
    print(f"   Exploitation rate: {exploitation_rate*100:.1f}% ({num_exploited}/{num_samples})")
    print(f"   Avg compression ratio: {avg_compression_ratio:.3f}x")
    print(f"   Total bit savings: {total_original_bits - total_compressed_bits:+.0f} bits")
    print(f"   Runtime: {runtime:.3f}s")
    ...
    if save_plot:
        ...
        ax = axes[1, 0]
        bit_savings = [r['bit_savings'] for r in results]
        ax.hist(bit_savings, bins=20, color='skyblue', edgecolor='black', alpha=0.7)
        ...
        ax.pie(exploit_count, labels=['Exploited', 'Ignored'], autopct='%1.1f%%',
              colors=['green', 'red'], startangle=90)
        ax.set_title(f'Symmetry Exploitation Rate: {exploitation_rate*100:.1f}%', fontsize=12)
...
Throughput:           {num_samples/runtime:.1f} vectors/sec
        ...
        plt.tight_layout()
        plt.savefig('benchmarks/results/compression_benchmark.png', dpi=150, bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--coherence-sweep', action='store_true', help='Run coherence sweep')
    parser.add_argument('--dimension-sweep', action='store_true', help='Run dimension sweep')
    parser.add_argument('--compression-test', action='store_true', help='Run compression test')
